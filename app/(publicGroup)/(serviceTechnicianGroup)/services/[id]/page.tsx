...
const ServiceByIdPage = async ({
  ...
  const reviewList = Array.isArray(serviceData.review) ? serviceData.review : [];
  const averageRating = reviewList.length
    ...
    : "0.0";
  const locations = Array.isArray(serviceData.location)
    ? serviceData.location
    : typeof serviceData.location === "string"
      ? [serviceData.location]
      : [];
  const availableSlots = Array.isArray(serviceData.availableAt)
    ? serviceData.availableAt
    : typeof serviceData.availableAt === "string"
      ? [serviceData.availableAt]
      : [];
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="flex flex-col gap-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 text-xs text-muted-foreground flex-wrap">
                          ...
                          <span>{reviewList.length ? `${reviewList.length} review${reviewList.length === 1 ? "" : "s"}` : "No reviews yet"}</span>
                        </div>
                      </div>
                    </div>
